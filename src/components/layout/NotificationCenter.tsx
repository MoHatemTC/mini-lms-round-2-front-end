...
import { 
  Bell, X, CheckCircle2, MessageSquare, 
  Award, Clock, Calendar, ShieldCheck
} from 'lucide-react';
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-border p-4 bg-muted/30">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setIsOpen(false)} className="p-1 rounded-lg hover:bg-muted text-text-secondary transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              </div>
              ...
              <div className="max-h-[400px] overflow-y-auto scrollbar-hide">
                {notifications.length === 0 ? (
                  <div className="p-8 text-center">
                    ...
                    <p className="text-xs text-text-secondary mt-1">Check back later for new updates.</p>
                  </div>
                ) : (
                    ...
                    {notifications.map((notification) => (
                      ...
                      >
                        ...
                        <div className="flex gap-4">
                          <div className={cn("h-10 w-10 shrink-0 rounded-xl flex items-center justify-center", notification.bg, notification.color)}>
                            <notification.icon className="h-5 w-5" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                              <p className={cn("text-sm font-bold truncate", notification.unread ? "text-foreground" : "text-foreground/80")}>
                                {notification.title}
                              </p>
              ...
              <div className="border-t border-border p-2 bg-muted/30">
                <Button variant="ghost" className="w-full text-xs font-semibold text-text-secondary hover:text-foreground">

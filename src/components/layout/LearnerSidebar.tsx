...
import { 
  LayoutDashboard, PlayCircle, BookOpen, Map, 
  Tags, ClipboardCheck, FileQuestion, Award, 
  Trophy, Bookmark, DownloadCloud, Calendar, 
...
const navigation = [
  { group: 'Learning', items: [
    ...
    { name: 'Continue Learning', href: '/learner/courses/1/play', icon: PlayCircle },
  ...
  { group: 'Assessments & Rewards', items: [
    { name: 'Assignments', href: '/learner/assignments', icon: ClipboardCheck },
    ...
    <div className="flex h-full flex-col bg-[#042F2E] dark:bg-[#020617] text-white">
      ...
      <div className="flex h-16 shrink-0 items-center px-6 border-b border-white/10">
        <Link to="/learner" className="flex items-center gap-3">
          ...
          {!isCollapsed && (
            <span className="text-xl font-bold tracking-tight text-white">Learnify</span>
          )}
        </Link>
      </div>
      ...
      {!isCollapsed && (
        <div className="px-6 py-5 border-b border-white/10">
          <div className="flex justify-between items-end mb-2">
            <span className="text-xs font-semibold text-white/50 uppercase tracking-wider">Weekly Goal</span>
            <span className="text-xs font-bold text-teal-400">3/5 Days</span>
          </div>
          <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-teal-400 to-emerald-500 rounded-full" style={{ width: '60%' }} />
          </div>
        </div>
      )}
          ...
          <div key={group.group}>
            ...
            <ul className="space-y-1">
              {group.items.map((item) => {
                ...
                const isSpecialActive = isActive && item.name === 'Continue Learning';
                  ...
                  <li key={item.name}>
                    ...
                    >
                      ...
                      <item.icon className={cn(
                        ...
                        isActive ? (isSpecialActive ? "text-teal-400" : "text-teal-400") : "text-white/40 group-hover:text-white/80"
                      )} />

    ...
    <Card className="overflow-hidden group flex flex-col h-full hover:border-primary/30 transition-colors">
      ...
      <CardContent className="p-5 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-2">
          <Badge variant="secondary" className="text-[10px] uppercase px-2 py-0 h-5">
            {course.category}
          </Badge>
        </div>
        ...
        <div className="mt-auto pt-4 space-y-4 border-t border-border">
          <div>
            <div className="flex justify-between text-xs font-medium mb-1.5">
              <span className="text-text-secondary">{course.completedLessons} / {course.totalLessons} lessons</span>
              <span className={isCompleted ? "text-success" : "text-primary"}>{course.progress}%</span>
            </div>
            <ProgressBar 
          ...
          <Button className="w-full" variant={isCompleted ? "outline" : "primary"}>
            {isCompleted ? "Review Course" : (
               <><Play className="mr-2 h-4 w-4" fill="currentColor" /> Continue Learning</>
            )}
          </Button>
        </div>
      </CardContent>

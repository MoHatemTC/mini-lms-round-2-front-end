...
import { 
  ...
  Image as ImageIcon, Video, BookOpen, FileQuestion, 
  ClipboardCheck, Award, UploadCloud, Check, ArrowUpRight
} from 'lucide-react';
    ...
    <div className="max-w-[1200px] mx-auto pb-12">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="h-10 px-4 rounded-xl border-border">Save Draft</Button>
          <Button className="btn-gradient h-10 px-6 rounded-xl text-white shadow-sm" onClick={() => navigate('/admin/courses')}>
            Publish Course
          </Button>
        </div>
      </div>
              ...
              return (
                ...
                >
                  <div className={cn(
                    ...
                    {isCompleted ? <CheckCircle2 className="h-4 w-4" /> : <step.icon className="h-3.5 w-3.5" />}
                  </div>
        ...
        <div className="flex-1 min-w-0">
          <Card className="p-0 border-border overflow-hidden bg-card shadow-sm rounded-[24px]">
              ...
              >
                ...
                {currentStep === 0 && (
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-sm text-text-secondary">Provide the core details about your course.</p>
                    </div>
                    
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        <Label htmlFor="title">Course Title</Label>
                        <Input id="title" placeholder="e.g. Advanced React Patterns & Architecture" className="h-11 rounded-xl bg-background border-border" />
                      </div>
                      
                      <div className="space-y-1.5">
                        <Label htmlFor="subtitle">Subtitle (Short Description)</Label>
                        <Input id="subtitle" placeholder="A brief summary that appears on the course card" className="h-11 rounded-xl bg-background border-border" />
                      </div>
                      ...
                      <div className="space-y-1.5 pt-2">
                        ...
                        <div className="border border-border rounded-xl overflow-hidden bg-background">
                          ...
                          <div className="flex items-center gap-1 p-2 border-b border-border bg-muted/30">
                            {['B', 'I', 'U'].map((format) => (
                              <button key={format} className="h-8 w-8 rounded flex items-center justify-center font-serif font-bold text-sm hover:bg-muted text-text-secondary hover:text-foreground">{format}</button>
                            ))}
                            <div className="w-px h-4 bg-border mx-1" />
                            <button className="h-8 px-2 rounded flex items-center justify-center text-sm hover:bg-muted text-text-secondary hover:text-foreground">&lt;/&gt;</button>
                          </div>
                          <textarea 
                            rows={6}
                ...
                {currentStep === 1 && (
                  <div className="space-y-6">
                    <div>
                      ...
                      <p className="text-sm text-text-secondary">Upload a cover image and promotional video.</p>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="space-y-3">
                        ...
                        <div className="border-2 border-dashed border-border rounded-2xl p-8 flex flex-col items-center justify-center bg-background hover:bg-muted/30 transition-colors cursor-pointer group">
                          ...
                          <p className="text-xs text-text-secondary">SVG, PNG, JPG or GIF (max. 1920x1080px)</p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="border border-border rounded-2xl p-6 bg-background flex items-center gap-4">
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-medium">Add a promo video</p>
                            <p className="text-xs text-text-secondary">Students who watch a promo video are 5X more likely to enroll.</p>
                          </div>
                          <Button variant="outline" className="rounded-xl h-9">Upload Video</Button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {currentStep > 1 && (
                  <div className="flex flex-col items-center justify-center h-[400px] text-center space-y-4">
                    ...
                    <h2 className="text-2xl font-bold">Launch {steps[currentStep].title} Builder</h2>
                    <p className="text-text-secondary max-w-md">
                      This opens the advanced visual builder for {steps[currentStep].title.toLowerCase()}. You'll be redirected to a dedicated workspace.
                    </p>
                    <Link to={`/admin/${steps[currentStep].title.toLowerCase().replace(' ', '-')}`}>
            ...
            <div className="flex items-center justify-between p-6 border-t border-border bg-muted/10">
              ...
              >
                {currentStep === steps.length - 1 ? 'Finish & Review' : 'Continue'}

      ...
      cell: () => (
        <div className="flex items-center justify-end gap-2 opacity-50 hover:opacity-100 transition-opacity">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 hover:text-danger hover:bg-danger/10"
            title="Delete course"
          >
      ...
      {viewMode === "table" ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {mockCourses.map((course) => (
            <Card
              key={course.id}
              className="overflow-hidden group flex flex-col h-full border-border hover:border-primary/50 transition-colors"
            >

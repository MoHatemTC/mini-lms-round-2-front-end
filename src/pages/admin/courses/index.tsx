...
import { 
  ...
  MoreHorizontal, Star, Clock, Users, BookOpen,
  Edit, Trash2, Eye, Copy, ArrowUpDown
} from 'lucide-react';
...
const mockCourses = [
  {
    ...
    instructor: { name: 'Sarah Chen', avatar: 'https://i.pravatar.cc/150?img=1' },
  ...
  },
  {
    ...
    instructor: { name: 'Marcus Johnson', avatar: 'https://i.pravatar.cc/150?img=3' },
    category: 'Design',
  ...
  },
  {
    ...
    instructor: { name: 'Dr. Alex Rivera', avatar: 'https://i.pravatar.cc/150?img=11' },
  ...
  },
  {
    ...
    instructor: { name: 'William Chen', avatar: 'https://i.pravatar.cc/150?img=8' },
    category: 'Development',
    difficulty: 'Intermediate',
    status: 'Review',
    students: 0,
  ...
  },
  {
    ...
    instructor: { name: 'Emma Wilson', avatar: 'https://i.pravatar.cc/150?img=9' },
  ...
  },
  {
    ...
    instructor: { name: 'Sophia Davis', avatar: 'https://i.pravatar.cc/150?img=5' },
    category: 'Marketing',
    difficulty: 'Beginner',
    status: 'Published',
    students: 5600,
    ...
    lessons: 22,
    duration: '8h 10m',
    image: 'https://images.unsplash.com/photo-1432888117426-1482f5f246bb?w=600&h=340&fit=crop',
    ...
    <div className="space-y-6 sm:space-y-8 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1">Manage your course catalog, curriculum, and settings.</p>
        </div>
          ...
          <div className="flex bg-muted p-1 rounded-xl">
            <button
              ...
              className={cn(
                ...
                viewMode === 'grid' ? "bg-background shadow-sm text-foreground" : "text-text-secondary hover:text-foreground"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "p-1.5 rounded-lg transition-colors",
                viewMode === 'table' ? "bg-background shadow-sm text-foreground" : "text-text-secondary hover:text-foreground"
              )}
            >
                ...
                <Card className="overflow-hidden group hover:shadow-premium transition-all duration-300 h-full flex flex-col">
                  ...
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex gap-2">
                      <Badge variant={getStatusColor(course.status)} className="shadow-sm border-0 font-medium">
                        {course.status}
                      </Badge>
                      <Badge variant="secondary" className="shadow-sm border-0 font-medium bg-black/50 text-white hover:bg-black/50 backdrop-blur-md">
                        {course.difficulty}
                      </Badge>
                    </div>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                      ...
                      <div className="flex items-center gap-2">
                        <Avatar src={course.instructor.avatar} alt={course.instructor.name} className="h-6 w-6" />
                        <span className="text-xs text-text-secondary">{course.instructor.name}</span>
                      </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-muted/50 text-text-secondary uppercase text-[10px] font-semibold tracking-wider">
                    <tr>
                      ...
                      <th className="px-6 py-4 cursor-pointer hover:text-foreground">
                        <div className="flex items-center gap-1">Enrollments <ArrowUpDown className="h-3 w-3" /></div>
                      </th>
                    ...
                    {mockCourses.map((course) => (
                      <tr key={course.id} className="hover:bg-muted/30 transition-colors group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <Badge variant={getStatusColor(course.status)} className="rounded-md font-medium border-0 bg-opacity-15">
                            {course.status}
                          </Badge>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-1 text-foreground">
                            ...
                            <span className="font-medium">{course.rating > 0 ? course.rating : '-'}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="p-2 text-text-secondary hover:text-primary hover:bg-primary/10 rounded-lg transition-colors" title="Preview">
                              <Eye className="h-4 w-4" />
                            </button>
                            <Link to={`/admin/courses/${course.id}/edit`}>
                              <button className="p-2 text-text-secondary hover:text-blue-500 hover:bg-blue-500/10 rounded-lg transition-colors" title="Edit">
      ...
      <div className="flex items-center justify-between border-t border-border pt-6">
        <p className="text-sm text-text-secondary">Showing <span className="font-medium text-foreground">1</span> to <span className="font-medium text-foreground">6</span> of <span className="font-medium text-foreground">24</span> results</p>

...
import { 
  Play, CheckCircle2, Search, Filter, 
  Grid, List as ListIcon, Clock, BookOpen,
  Award, Trophy
} from 'lucide-react';
    ...
    <div className="space-y-6 sm:space-y-8 pb-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary mt-1">Pick up where you left off or review completed courses.</p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            {['all', 'in-progress', 'completed'].map((filter) => (
              <button
                ...
                className={cn(
                  ...
                  activeFilter === filter ? "bg-background shadow-sm text-foreground" : "text-text-secondary hover:text-foreground"
                )}
              >
                {filter.replace('-', ' ')}
              </button>
          ...
          <div className="hidden sm:flex bg-muted p-1 rounded-xl">
            <button
              ...
              className={cn("p-1.5 rounded-lg transition-colors", viewMode === 'grid' ? "bg-background shadow-sm text-foreground" : "text-text-secondary hover:text-foreground")}
            >
              <Grid className="h-4 w-4" />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={cn("p-1.5 rounded-lg transition-colors", viewMode === 'list' ? "bg-background shadow-sm text-foreground" : "text-text-secondary hover:text-foreground")}
            >
                ...
                <div className={cn(
                  ...
                  <div className="absolute top-3 left-3 z-20">
                    {course.status === 'completed' ? (
                      <Badge variant="success" className="bg-success text-white border-0 shadow-sm">
                        <CheckCircle2 className="h-3 w-3 mr-1" /> Completed
                      </Badge>
                    ) : (
                      <Badge variant="secondary" className="bg-black/50 text-white backdrop-blur-md border-0 shadow-sm">
                        {course.category}
                      </Badge>
                    )}
                  </div>
                ...
                <div className={cn(
                  ...
                  <div className="mt-auto space-y-3">
                    <div className="flex items-center justify-between text-xs font-medium">
                      <span className={course.progress === 100 ? "text-success" : "text-text-secondary"}>
                        {course.progress}% Completed
                      </span>
                      <span className="text-text-secondary">{course.lessonsCompleted} / {course.totalLessons} Lessons</span>
                    </div>
                    ...
                    {viewMode === 'grid' && (
                      <div className="pt-3 flex items-center justify-between">
                        <Link to={`/learner/courses/${course.id}`} className="text-xs font-semibold text-primary hover:text-primary/80 transition-colors">
                          View Details
                        </Link>
                        {course.progress === 100 && (

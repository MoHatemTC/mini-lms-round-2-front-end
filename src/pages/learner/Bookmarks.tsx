...
import { 
  Bookmark, LayoutGrid, List, Search, Filter, 
  MoreVertical, PlayCircle, Clock, Star, Share2, Trash2
} from 'lucide-react';
            ...
            {['all', 'course', 'lesson', 'article'].map(type => (
              <button 
                ...
                className={cn(
                  ...
                  filterType === type ? "bg-background text-foreground shadow-sm" : "text-text-secondary hover:text-foreground"
                )}
              >
                {type}s
              </button>
          ...
          {filteredBookmarks.map((item) => (
            ...
            >
              <Card className={cn(
                ...
                viewMode === 'grid' ? "flex-col h-full" : "flex-row h-32 sm:h-40"
              )}>
                {/* Thumbnail */}
                <div className={cn(
                  "relative overflow-hidden shrink-0",
                  viewMode === 'grid' ? "aspect-video w-full" : "w-32 sm:w-48 h-full"
                )}>
                  <img 
                    src={item.thumbnail} 
                    alt={item.title} 
                  ...
                  <div className="absolute top-3 left-3 flex gap-2">
                    <Badge variant="secondary" className="bg-background/90 text-foreground backdrop-blur-sm border-0 capitalize px-2 py-0.5">
                      {item.type}
                    </Badge>
                  </div>
                ...
                <div className={cn(
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <div className="text-xs font-medium text-primary uppercase tracking-wider">{item.category}</div>
                    {viewMode === 'list' && (
                  ...
                  <p className="text-sm text-text-secondary mb-3">{item.instructor}</p>
                  ...
                  <div className="mt-auto">
                    ...
                    {item.type === 'course' && (
                      <div className="mb-4">
                        <div className="flex justify-between text-xs mb-1">
                          <span className="font-medium text-text-secondary">{item.progress}% Complete</span>
                        </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" className="btn-gradient rounded-lg text-white shadow-sm h-8 px-3">
          ...
          {filteredBookmarks.length === 0 && (
            <div className={cn(
              ...
              <p className="text-text-secondary max-w-sm mb-6">
                You haven't saved any {filterType === 'all' ? 'items' : filterType + 's'} yet. Browse the catalog to find interesting content.
              </p>
